...
object DistrictUtils {
    ...
    fun getDistrictNames(config: Config, features: List<GeojsonFeature>): List<String> {
    ...
    fun filterByDistrictName(
        ...
        features.filter { node -> node.properties.get(config.idProp)?.toString() == districtName }
}